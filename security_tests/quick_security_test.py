...
ADMIN_USER = {'username': 'admin', 'password': 'adminpassword'}  # Update with valid credentials
NON_ADMIN_USER = {'username': 'buyer1', 'password': 'buyerpassword'}  # Update with valid credentials
...
class QuickSecurityTester:
    ...
    def login(self, session, username, password):
        ...
        if not csrf_token:
            logger.error(f"Failed to obtain CSRF token for {username}")
            return False
    ...
    def test_access_control(self):
        ...
        if not self.login(self.non_admin_session, NON_ADMIN_USER['username'], NON_ADMIN_USER['password']):
        ...
        for url in admin_urls:
            ...
            if response.status_code in [302, 303, 307, 403]:
                logger.info(f"✓ Access correctly denied to {url} (Status: {response.status_code})")
                results.append({
                    'url': url,
                    'status': 'PASS',
                ...
                logger.warning(f"✗ VULNERABILITY: Access control failure on {url} (Status: {response.status_code})")
                results.append({
                    'url': url,
                    'status': 'FAIL',
                    'code': response.status_code,
        ...
        if failed > 0:
            logger.warning(f"⚠️ Found {failed} access control vulnerabilities")
        else:
            logger.info("✓ All access control tests passed successfully")
    ...
    def test_csrf_protection(self):
        ...
        if not self.login(self.admin_session, ADMIN_USER['username'], ADMIN_USER['password']):
        ...
        forms_to_test = [
            {
                ...
                'post_data': {'amount': '100', 'description': 'CSRF Test'}
            },
            {
                'url': '/inventory/add/',
                'post_data': {'name': 'CSRF Test Item', 'quantity': '10'}
            },
            {
        ...
        for form in forms_to_test:
            ...
            if response.status_code == 403:
                logger.info(f"✓ CSRF protection working correctly on {url}")
                results.append({
                    'url': url,
                    'status': 'PASS',
                    'code': response.status_code,
                ...
                logger.warning(f"✗ VULNERABILITY: CSRF protection may be missing on {url} (Status: {response.status_code})")
                results.append({
                    'url': url,
                    'status': 'FAIL',
                    'code': response.status_code,
        ...
        if response.status_code == 200:
            ...
            if csrf_input:
                ...
                if response.status_code == 403:
                    logger.info("✓ Invalid CSRF token correctly rejected")
                    results.append({
                        'url': form_url,
                        'status': 'PASS',
                        'code': response.status_code,
                    ...
                    logger.warning(f"✗ VULNERABILITY: Form accepted invalid CSRF token (Status: {response.status_code})")
                    results.append({
                        'url': form_url, 
                        'status': 'FAIL',
                        'code': response.status_code,
        ...
        if failed > 0:
            logger.warning(f"⚠️ Found {failed} CSRF vulnerabilities")
        else:
            logger.info("✓ All CSRF protection tests passed successfully")
    ...
    def run_tests(self):
        ...
        if (isinstance(access_results, list) and access_failed > 0) or (isinstance(csrf_results, list) and csrf_failed > 0):
            logger.warning("\n⚠️ Security vulnerabilities detected! Review the log for details.")
        else:
